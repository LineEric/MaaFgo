...
> **状态**：设计决策已确认；行锚点离线原型已落地，后续继续开发
> **版本**：v0.1
> **日期**：2026-08-04
> **分支**：`feat/support-selection`
...
## 22. 2026-08-04 已确认设计决策

本节记录本轮讨论后已经确认的第一版边界；若与前文的早期推荐或待确认内容冲突，以本节为准。

### 22.1 行定位

- 使用右侧“助战编入确认”按钮 TemplateMatch 结果作为主行锚点；
- 根据按钮 Y 坐标动态推导同一行的头像、等级、从者信息和宝具信息 ROI，不写死第 1/2/3 行坐标；
- 只处理头像、等级、从者信息、宝具信息和确认按钮均完整可见的记录；
- 顶部或底部截断记录在当前屏跳过；
- Pipeline Swipe 保留约一行重叠，使截断记录在下一屏完整出现；
- 第一版不增加卡片边框或轮廓备用定位，只有真机验证发现按钮锚点不稳定时再评估。
...
程序不做开放式全从者识别，也不在运行时回答“当前头像是谁”。程序只需要判断当前完整行是否属于用户指定的目标。

同一目标可以包含多个再临、灵衣或用户认为等价的立绘版本。例如岸波白野的男女版本可以归入同一可接受目标集合，不要求按图鉴 ID 强制区分。

### 22.3 从者身份识别

第一版采用目标限定头像匹配：

1. Pipeline 先切换到用户指定职阶；
2. 任务启动时只加载用户指定从者的头像集合；
3. Custom Recognition 只在当前完整行的头像 ROI 中匹配该目标集合；
4. 任一目标头像达到标定阈值后，继续检查等级和宝具条件；
5. 从者名称 OCR 不作为第一版身份通过条件，后续可作为日志或辅助证据。

头像资源可以来自用户本地已有的统一 130×130 PNG 目录，但路径必须可配置，不能硬编码某台机器的绝对路径。MaaFgo 只复用图片资源，自行维护目标映射、加载、缓存、裁切和 TemplateMatch 实现，不参考或复用 BBchannel 的识别代码。

正式运行只加载当前目标的少量模板，不遍历完整头像库。例如目标 C.I.E.L. 有 6 张头像、当前屏有 2 条完整记录时，只进行约 2×6 组目标比较。

参考截图离线验证中：

- 顶部截断记录与底部完整记录均由“谜之代行者C.I.E.L_3.png”获得目标集合最高分；
- 底部完整记录与 6 张 C.I.E.L. 头像比较时，正确模板明显领先其他版本；
- 在 1741 张头像的验证性全库排名中，正确模板排名第 1；
- 全库排名只用于验证头像资源质量，不进入正式运行流程。

### 22.4 MaaFramework 架构边界
...
Custom Recognition 不获取新截图，不滑动、不点击、不刷新，也不驱动完整任务状态机。

### 22.5 性能边界

- 目标头像在任务启动时加载并缓存，扫描过程中不重复读取磁盘；
- 只匹配用户当前指定目标的头像集合；
- 不对全部头像执行逐行分类；
- 找到第一个满足全部条件的候选后停止当前扫描；
- 滑动和刷新均保留硬上限，不能无限循环。
...
- 从者等级 OCR 的稳定性策略；
- 宝具等级 OCR 的稳定性策略；
- OCR ROI、颜色过滤、缩放和多路一致性规则；
- Pipeline 获取新截图后的有限 OCR 重试次数；
- 数字纠错范围与误选/漏选验收标准；
- first_match 是否最终采用；
- 好友/非好友限制；
- 每轮最大滑动次数和最大刷新次数；
...
下一次继续开发时，先实现“用户指定从者＋用户指定职阶”的配置契约和目标头像资源加载，再编写目标头像集合的有限匹配离线测试。等级与宝具 OCR 在获得更多真实截图后继续讨论。
